...
public final class AsyncHttpManager {
	...
	private static class AsyncHttpSenderTask extends AsyncTask<Void, Void, OutputHolder>{
		...
		protected OutputHolder doInBackground(Void... arg0) {
			...
			try {
				...
				HttpResponse response = AsyncHttpManager.getDefaultHttpClient().execute(httpUriRequest);
	...
	public static void setConnection_timeout(int connection_timeout) {
		AsyncHttpManager.connection_timeout = connection_timeout;
		isTimeChange = true;
	}
	...
	public static void setSocket_timeout(int socket_timeout) {
		AsyncHttpManager.socket_timeout = socket_timeout;
		isTimeChange = true;
	}
